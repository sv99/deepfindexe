...
func Fits(in []byte) bool {
	return len(in) > 30 &&bytes.HasPrefix(in, []byte{
...
func Xar(in []byte) bool {
	return len(in) > 4 && bytes.HasPrefix(in, []byte{0x78, 0x61, 0x72, 0x21})
}

// Bz2 matches a bzip2 file.
func Bz2(in []byte) bool {
	return len(in) > 3 && bytes.HasPrefix(in, []byte{0x42, 0x5A, 0x68})
}

// Ar matches an ar (Unix) archive file.
func Ar(in []byte) bool {
	return len(in) > 6 && bytes.HasPrefix(in, []byte{0x21, 0x3C, 0x61, 0x72, 0x63, 0x68, 0x3E})
}

// Deb matches a Debian package file.
func Deb(in []byte) bool {
	return len(in) > 26 && bytes.HasPrefix(in[8:], []byte{
...
func Rar(in []byte) bool {
	return len(in) > 7 && bytes.HasPrefix(in, []byte("Rar!\x1A\x07\x00")) ||
		bytes.HasPrefix(in, []byte("Rar!\x1A\x07\x01\x00"))
}

// Warc matches a Web ARChive file.
func Warc(in []byte) bool {
	return len(in) > 5 && bytes.HasPrefix(in, []byte("WARC/"))
}
...
func Cab(in []byte) bool {
	return len(in) > 4 && bytes.HasPrefix(in, []byte("MSCF"))
}
...
func Xz(in []byte) bool {
	return len(in) > 6 && bytes.HasPrefix(in, []byte{0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00})
}

func Lzip(in []byte) bool {
	return len(in) > 4 && in[0] == 0x4C && in[1] == 0x5A && in[2] == 0x49 && in[3] == 0x50
}

// Lz4 matches an xz compressed stream based on https://github.com/lz4/lz4/blob/master/doc/lz4_Frame_format.md.
func Lz4(in []byte) bool {
	return len(in) > 4 && bytes.HasPrefix(in, []byte{0x04, 0x22, 0x4D, 0x18})
}

// Snappy matches an snappy compressed stream based on https://tukaani.org/xz/xz-file-format.txt.
func Snappy(in []byte) bool {
	return len(in) > 10 && bytes.HasPrefix(in[4:], []byte("sNaPpY"))
}
